...
const countryCodeMap = {
  ...
  'Ivory Coast': 'CI', 'Swaziland': 'SZ', 'Cape Verde': 'CV', 'Czechia': 'CZ',
  "C\u00f4te d'Ivoire": 'CI',
};

const citiesAll = JSON.parse(readFileSync(join(root, 'node_modules', 'cities.json', 'cities.json'), 'utf-8'));
...
const index = {};
for (const [countryName, iso2] of Object.entries(countryCodeMap)) {
  const cities = [...(byCountry[iso2] || [])].sort((a, b) => a.localeCompare(b));
  index[countryName] = iso2;
  writeFileSync(join(outDir, `${iso2}.json`), JSON.stringify(cities), 'utf-8');
  console.log(`${countryName} (${iso2}): ${cities.length} cities`);
}

writeFileSync(join(outDir, '_index.json'), JSON.stringify(index, null, 2), 'utf-8');
console.log(`\nDone. Generated ${Object.keys(countryCodeMap).length} country files + _index.json`);
